...
namespace Palmfit.Core.Implementations
{
    public class FileUploadRepository : IFileUploadRepository
    {
        ...
        public async Task<string> UploadImageToCloudinaryAndSave(IFormFile file)
        {
            ...
            const int maxFileSizeInBytes = 500 * 1024;
            ...
            var fileUploadModelDto = new FileUploadModelDto
            {
                ImageName = file.FileName,
                CloudinaryPublicId = uploadResult.PublicId,
                CloudinaryUrl = uploadResult.Uri.ToString()
            };
            var fileUploadEntity = ConvertDtoToEntity(fileUploadModelDto);

            _palmfitDb.fileUploadmodels.Add(fileUploadEntity);
            _palmfitDb.SaveChanges();

            return "File uploaded and saved successfully!";
        }

        public FileUploadModel ConvertDtoToEntity(FileUploadModelDto dto)
        {
